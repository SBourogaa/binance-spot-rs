 ...
 * - `quote_order_quantity`: Order quantity in quote asset.
 * - `price`: Order price.
...
impl SorOrderSpec<Unvalidated> {
     ...
     */
    pub fn build(self) -> Result<SorOrderSpec<Validated>> {
        self.validate().context("Failed to validate SorOrderSpec")?;
