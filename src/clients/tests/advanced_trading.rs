...
mod tests {
    ...
    use crate::{
        clients::{
            r#trait::{TradingClient, TickerClient, GeneralClient},
            tests::helpers::*,
        },
        types::{
            requests::{
            ...
            responses::{Order, TestOrder, SymbolInfo, OrderList},
        },
        errors::{BinanceError, ErrorCategory, RequestError},
        enums::{OrderSide, OrderType, TimeInForce, OrderListOrderStatus, OrderStatus},
    ...
    async fn get_sor_enabled_symbol() -> Option<String> {
        ...
        let exchange_info_spec = ExchangeInfoSpec::new().build().expect("Exchange info spec validation");
        let exchange_info = with_timeout(rest_client.exchange_info(exchange_info_spec)).await.expect("Get exchange info");
    ...
    fn assert_valid_sor_order(order: &Order) {
        ...
        if let Some(fills) = &order.fills {
            for fill in fills {
                ...
                assert!(fill.quantity > Decimal::ZERO, "Fill quantity should be positive");
                assert!(fill.price > Decimal::ZERO, "Fill price should be positive");
                assert!(!fill.commission_asset.is_empty(), "Commission asset should not be empty");
            }
        }
    }
    ...
    fn assert_valid_sor_test_order(test_order: &TestOrder) {
        if let Some(commission) = &test_order.standard_commission_for_order {
            assert!(commission.maker >= Decimal::ZERO, "Maker commission should be non-negative");
            assert!(commission.taker >= Decimal::ZERO, "Taker commission should be non-negative");
        }
    }
    ...
    fn assert_valid_oco_order_list(order_list: &OrderList) {
        assert!(order_list.order_list_id > 0, "Order list ID should be positive");
        assert!(!order_list.symbol.is_empty(), "Symbol should not be empty");
        assert!(!order_list.orders.is_empty(), "Order list should contain orders");
        
        assert_eq!(order_list.orders.len(), 2, "OCO should have exactly 2 orders");
    ...
    async fn cleanup_all_open_order_lists() {
        ...
        if cancelled_count > 0 {
            println!("🧹 Cleanup complete: {}/{} order lists cancelled.", cancelled_count, open_lists.len());
        }
    }
    ...
    async fn test_place_sor_order_limit() {
        ...
        let price_spec = TickerPriceSpec::new().with_symbol(&sor_symbol).build().expect("Price spec validation");
        let current_prices = with_timeout(rest_client.ticker_price(price_spec)).await.expect("Get current price");
        let market_price = current_prices[0].price;
        
        let info_spec = ExchangeInfoSpec::new().with_symbol(&sor_symbol).build().expect("Info spec validation");
        let exchange_info = with_timeout(rest_client.exchange_info(info_spec)).await.expect("Get exchange info");
        ...
        let rest_order = with_timeout(rest_client.place_sor_order(rest_spec)).await.expect("REST place SOR order");
        let ws_order = with_timeout(ws_client.place_sor_order(ws_spec)).await.expect("WebSocket place SOR order");
    ...
    async fn test_test_sor_order_limit() {
        ...
        let rest_test_order = with_timeout(rest_client.test_sor_order(rest_spec)).await.expect("REST test SOR order");
        let ws_test_order = with_timeout(ws_client.test_sor_order(ws_spec)).await.expect("WebSocket test SOR order");
        ...
        if let Some(commission) = &rest_test_order.standard_commission_for_order {
            assert!(commission.maker >= Decimal::ZERO, "Commission rates should be non-negative");
            assert!(commission.taker >= Decimal::ZERO, "Commission rates should be non-negative");
        }
    ...
    async fn test_test_sor_order_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(test_order) => panic!("Expected {} error, got successful response: {:?}", client_name, test_order),
                Err(err) => {
    ...
    async fn test_place_oco_order_take_profit_stop_loss() {
        ...
        let price_spec = TickerPriceSpec::new().with_symbol(test_symbol).build().expect("Price spec validation");
        let current_prices = with_timeout(rest_client.ticker_price(price_spec)).await.expect("Get current price");
        let market_price = current_prices[0].price;
        
        let info_spec = ExchangeInfoSpec::new().with_symbol(test_symbol).build().expect("Info spec validation");
        let exchange_info = with_timeout(rest_client.exchange_info(info_spec)).await.expect("Get exchange info");
        let symbol_info = &exchange_info.symbols[0];
        
        let (_, safe_quantity) = calculate_safe_order_params(market_price, symbol_info);
        
        let take_profit_price = make_price_tick_compliant(market_price * Decimal::new(105, 2), symbol_info);
        let stop_loss_price = make_price_tick_compliant(market_price * Decimal::new(90, 2), symbol_info);
        ...
        let rest_order_list = with_timeout(rest_client.place_oco_order(rest_spec)).await.expect("REST place OCO order");
        let ws_order_list = with_timeout(ws_client.place_oco_order(ws_spec)).await.expect("WebSocket place OCO order");
    ...
    async fn test_place_oco_order_limit_maker_stop_loss() {
        ...
        let limit_maker_price = make_price_tick_compliant(market_price * Decimal::new(105, 2), symbol_info);
        let stop_loss_price = make_price_tick_compliant(market_price * Decimal::new(90, 2), symbol_info);
    ...
    async fn test_place_oco_order_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(order_list) => panic!("Expected {} error, got successful response: {:?}", client_name, order_list),
                Err(err) => {
                    let downcast = err.downcast_ref::<BinanceError>();
                    assert!(
                        matches!(
    ...
    async fn test_place_oto_order_limit_market() {
        ...
        let (safe_price, safe_quantity) = calculate_safe_order_params(market_price, symbol_info);
        let min_notional = get_min_notional(symbol_info);
        let contingent_quote_qty = round_to_quote_precision(min_notional * Decimal::new(15, 1), symbol_info);
        ...
        let rest_order_list = with_timeout(rest_client.place_oto_order(rest_spec)).await.expect("REST place OTO order");
        let ws_order_list = with_timeout(ws_client.place_oto_order(ws_spec)).await.expect("WebSocket place OTO order");
    ...
    async fn test_place_oto_order_limit_limit() {
        ...
        let (safe_price, safe_quantity) = calculate_safe_order_params(market_price, symbol_info);
        let contingent_price = make_price_tick_compliant(market_price * Decimal::new(102, 2), symbol_info);
    ...
    async fn test_place_otoco_order_limit_take_profit_stop_loss() {
        ...
        let (working_price, working_quantity) = calculate_safe_order_params(market_price, symbol_info);
        let take_profit_price = make_price_tick_compliant(market_price * Decimal::new(105, 2), symbol_info);
        let stop_loss_price = make_price_tick_compliant(market_price * Decimal::new(90, 2), symbol_info);
        ...
        let rest_order_list = with_timeout(rest_client.place_otoco_order(rest_spec)).await.expect("REST place OTOCO order");
        let ws_order_list = with_timeout(ws_client.place_otoco_order(ws_spec)).await.expect("WebSocket place OTOCO order");
        
        // Assert
        assert!(rest_order_list.order_list_id > 0, "Order list ID should be positive");
        assert_eq!(rest_order_list.symbol, test_symbol);
        assert_eq!(rest_order_list.orders.len(), 3, "OTOCO should have exactly 3 orders");
        
        assert!(ws_order_list.order_list_id > 0, "Order list ID should be positive");
        assert_eq!(ws_order_list.symbol, test_symbol);
        assert_eq!(ws_order_list.orders.len(), 3, "OTOCO should have exactly 3 orders");
    ...
    async fn test_cancel_order_list_oco() {
        ...
        let rest_order_list = with_timeout(rest_client.place_oco_order(rest_oco_spec)).await.expect("REST place OCO order");
        let ws_order_list = with_timeout(ws_client.place_oco_order(ws_oco_spec)).await.expect("WebSocket place OCO order");
        ...
        let rest_cancelled = with_timeout(rest_client.cancel_order_list(rest_cancel_spec)).await.expect("REST cancel order list");
        let ws_cancelled = with_timeout(ws_client.cancel_order_list(ws_cancel_spec)).await.expect("WebSocket cancel order list");
        ...
        assert_eq!(rest_cancelled.list_order_status, OrderListOrderStatus::AllDone);
        assert_eq!(ws_cancelled.list_order_status, OrderListOrderStatus::AllDone);
    ...
    async fn test_order_list_status_by_id() {
        ...
        let rest_status = with_timeout(rest_client.order_list_status(rest_status_spec)).await.expect("REST order list status");
        let ws_status = with_timeout(ws_client.order_list_status(ws_status_spec)).await.expect("WebSocket order list status");
        ...
        assert_eq!(rest_status.list_order_status, OrderListOrderStatus::Executing);
    ...
    async fn test_all_order_lists_default() {
        ...
        let rest_all_lists = with_timeout(rest_client.all_order_lists(rest_all_spec)).await.expect("REST all order lists");
        let ws_all_lists = with_timeout(ws_client.all_order_lists(ws_all_spec)).await.expect("WebSocket all order lists");
        
        // Assert
        assert_valid_order_lists(&rest_all_lists);
        assert_valid_order_lists(&ws_all_lists);
        
        let rest_contains_our_order = rest_all_lists.iter().any(|ol| ol.order_list_id == rest_order_list.order_list_id);
        let ws_contains_our_order = ws_all_lists.iter().any(|ol| ol.order_list_id == ws_order_list.order_list_id);
        assert!(rest_contains_our_order, "REST all order lists should contain our created order list");
        assert!(ws_contains_our_order, "WebSocket all order lists should contain our created order list");
    ...
    async fn test_all_order_lists_with_limit() {
        ...
        if !rest_all_lists.is_empty() {
            assert!(rest_all_lists.len() <= 5, "REST result should respect the limit of 5");
        }
        if !ws_all_lists.is_empty() {
            assert!(ws_all_lists.len() <= 5, "WebSocket result should respect the limit of 5");
        }
    ...
    async fn test_open_order_lists_default() {
        ...
        let rest_open_lists = with_timeout(rest_client.open_order_lists(rest_open_spec)).await.expect("REST open order lists");
        let ws_open_lists = with_timeout(ws_client.open_order_lists(ws_open_spec)).await.expect("WebSocket open order lists");
    ...
    async fn test_open_order_lists_with_active_order() {
        ...
        let rest_order_list = with_timeout(rest_client.place_oco_order(rest_oco_spec)).await.expect("REST OCO order creation");
        ...
        let rest_contains_our_order = rest_open_lists.iter().any(|ol| ol.order_list_id == rest_order_list.order_list_id);
        assert!(rest_contains_our_order, "REST open order lists should contain our created order list");
    ...
    async fn test_open_order_lists_behavior() {
        ...
        let rest_open_lists1 = with_timeout(rest_client.open_order_lists(rest_open_spec1)).await.expect("REST open order lists 1");
        let rest_open_lists2 = with_timeout(rest_client.open_order_lists(rest_open_spec2)).await.expect("REST open order lists 2");
        let ws_open_lists = with_timeout(ws_client.open_order_lists(ws_open_spec)).await.expect("WebSocket open order lists");
