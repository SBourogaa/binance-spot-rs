        ...
        Err(e) => {
            if let Some(api_error) = e.downcast_ref::<crate::BinanceError>() {
                if let Some(code) = api_error.api_code() {
                    assert_eq!(code, expected_code, "Expected error code {}, got {}", expected_code, code);
                } else {
                    panic!("Expected API error with code {}, got non-API error: {}", expected_code, e);
                }
            } else {
                panic!("Expected Binance API error, got: {}", e);
            }
        }
        Ok(_) => panic!("Expected error with code {}, but operation succeeded", expected_code)
    }
}