...
mod tests {
    ...
    use crate::{
        ...
        types::{
            ...
            responses::{Order, TestOrder, CancelledOrder, CancelReplaceOrder, AmendedOrder, SymbolInfo},
        },
        errors::{BinanceError, ErrorCategory, RequestError},
        enums::{OrderSide, OrderType, TimeInForce, OrderResponseType, OrderStatus, CancelRestrictions, CancelReplaceMode},
    ...
    fn assert_valid_cancelled_order(cancelled_order: &CancelledOrder) {
        match cancelled_order {
            ...
            CancelledOrder::OrderList(order_list) => {
                assert!(!order_list.orders.is_empty(), "Order list should not be empty");
                for order_summary in &order_list.orders {
                    assert!(order_summary.order_id > 0, "Order ID should be positive");
                    assert!(!order_summary.symbol.is_empty(), "Symbol should not be empty");
                }
            }
        }
    }
    ...
    fn assert_valid_amended_order(amended_order: &AmendedOrder) {
        assert!(amended_order.transaction_time > 0, "Transaction time should be positive");
        assert!(amended_order.execution_id > 0, "Execution ID should be positive");
    ...
    async fn test_place_order_limit() {
        ...
        let rest_order = with_timeout(rest_client.place_order(rest_spec)).await.expect("REST place order");
        let ws_order = with_timeout(ws_client.place_order(ws_spec)).await.expect("WebSocket place order");
    ...
    async fn test_test_order_limit() {
        ...
        let rest_test_order = with_timeout(rest_client.test_order(rest_spec)).await.expect("REST test order");
        let ws_test_order = with_timeout(ws_client.test_order(ws_spec)).await.expect("WebSocket test order");
    ...
    async fn test_cancel_all_orders_default() {
        ...
        let rest_order = with_timeout(rest_client.place_order(rest_order_spec)).await.expect("REST place order");
        let ws_order = with_timeout(ws_client.place_order(ws_order_spec)).await.expect("WebSocket place order");
        
        // Act
        let rest_cancel_spec = CancelAllOrdersSpec::new(test_symbol).build().expect("Cancel all spec validation");
        let ws_cancel_spec = CancelAllOrdersSpec::new(test_symbol).build().expect("Cancel all spec validation");

        let rest_cancelled_orders = with_timeout(rest_client.cancel_all_orders(rest_cancel_spec)).await.expect("REST cancel all orders");
        
        let ws_result = with_timeout(ws_client.cancel_all_orders(ws_cancel_spec)).await;
        
        // Assert
        assert!(!rest_cancelled_orders.is_empty(), "REST should cancel at least some orders");
        ...
        assert!(rest_cancelled_ids.contains(&rest_order.order_id), "REST should cancel its placed order");
        assert!(rest_cancelled_ids.contains(&ws_order.order_id), "REST should also cancel WebSocket's order (same account)");
        
        match ws_result {
            Ok(ws_cancelled_orders) => {
                assert!(ws_cancelled_orders.is_empty(), "WebSocket should return empty since REST already cancelled everything");
            }
            Err(err) => {
                let downcast = err.downcast_ref::<BinanceError>();
                assert!(
    ...
    async fn test_cancel_all_orders_no_orders() {
        ...
        match rest_result {
            Ok(cancelled_orders) => {
                assert!(cancelled_orders.is_empty(), "REST should return empty when no orders exist");
            }
            Err(err) => {
                let downcast = err.downcast_ref::<BinanceError>();
                assert!(
        ...
        match ws_result {
            Ok(cancelled_orders) => {
                assert!(cancelled_orders.is_empty(), "WebSocket should return empty when no orders exist");
            }
            Err(err) => {
                let downcast = err.downcast_ref::<BinanceError>();
                assert!(
    ...
    async fn test_cancel_all_orders_multiple_orders() {
        ...
        let rest_order1 = with_timeout(rest_client.place_order(rest_order1_spec)).await.expect("REST place order 1");
        let rest_order2 = with_timeout(rest_client.place_order(rest_order2_spec)).await.expect("REST place order 2");
        let ws_order = with_timeout(ws_client.place_order(ws_order_spec)).await.expect("WebSocket place order");
        ...
        // Assert
        assert!(!rest_cancelled_orders.is_empty(), "REST should cancel orders");
        ...
        assert!(rest_cancelled_ids.contains(&rest_order1.order_id), "REST should cancel its first order");
        assert!(rest_cancelled_ids.contains(&rest_order2.order_id), "REST should cancel its second order");
        assert!(rest_cancelled_ids.contains(&ws_order.order_id), "REST should cancel WebSocket's order (same account)");
    ...
    async fn test_cancel_all_orders_invalid_symbol() {
        ...
        // Act
        let rest_spec = CancelAllOrdersSpec::new("INVALID").build().expect("Cancel all spec validation");
        let ws_spec = CancelAllOrdersSpec::new("INVALID").build().expect("Cancel all spec validation");
    ...
    async fn test_cancel_replace_order_stop_on_failure() {
        ...
        let rest_initial_order = with_timeout(rest_client.place_order(rest_initial_spec)).await.expect("REST place initial order");
        let ws_initial_order = with_timeout(ws_client.place_order(ws_initial_spec)).await.expect("WebSocket place initial order");
        ...
        let rest_cancel_replace = with_timeout(rest_client.cancel_replace_order(rest_cancel_replace_spec)).await.expect("REST cancel replace order");
        let ws_cancel_replace = with_timeout(ws_client.cancel_replace_order(ws_cancel_replace_spec)).await.expect("WebSocket cancel replace order");
    ...
    async fn test_cancel_replace_order_with_restrictions() {
        ...
        let rest_result = with_timeout(rest_client.cancel_replace_order(rest_cancel_replace_spec)).await;
    ...
    async fn test_cancel_replace_order_market_new_order() {
        ...
        let (initial_price, safe_quantity) = calculate_safe_order_params(market_price, symbol_info);
        let min_notional = get_min_notional(symbol_info);
        let quote_quantity = round_to_quote_precision(min_notional * Decimal::new(15, 1), symbol_info);
        let base_id = chrono::Utc::now().timestamp_millis();
    ...
    async fn test_cancel_replace_order_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(cancel_replace) => panic!("Expected {} error, got successful response: {:?}", client_name, cancel_replace),
                Err(err) => {
                    let downcast = err.downcast_ref::<BinanceError>();
                    assert!(
                        matches!(
    ...
    async fn test_amend_order_basic() {
        ...
        let rest_amended_order = with_timeout(rest_client.amend_order(rest_amend_spec)).await.expect("REST amend order");
        let ws_amended_order = with_timeout(ws_client.amend_order(ws_amend_spec)).await.expect("WebSocket amend order");
        ...
        assert_eq!(rest_amended_order.amended_order.order_id, rest_initial_order.order_id);
        assert_eq!(ws_amended_order.amended_order.order_id, ws_initial_order.order_id);
        assert_eq!(rest_amended_order.amended_order.status, Some(OrderStatus::New));
        assert_eq!(ws_amended_order.amended_order.status, Some(OrderStatus::New));
    ...
    async fn test_amend_order_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(amended_order) => panic!("Expected {} error, got successful response: {:?}", client_name, amended_order),
                Err(err) => {
                    let downcast = err.downcast_ref::<BinanceError>();
                    assert!(
                        matches!(
