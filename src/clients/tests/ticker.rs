...
mod tests {
    use crate::{
        clients::{tests::helpers::*, r#trait::TickerClient},
    ...
    fn assert_valid_ticker_book(book: &TickerBook) {
        ...
        assert!(
            book.bid_price > Decimal::ZERO,
            "Bid price should be positive"
        );
        assert!(
            book.ask_price > Decimal::ZERO,
            "Ask price should be positive"
        );
        assert!(
            book.bid_quantity > Decimal::ZERO,
            "Bid quantity should be positive"
        );
        assert!(
            book.ask_quantity > Decimal::ZERO,
    ...
    fn assert_valid_ticker_statistics(stats: &TickerStatistics) {
        match stats {
            TickerStatistics::Full(full) => {
                ...
                assert!(
                    full.last_price > Decimal::ZERO,
                    "Last price should be positive"
                );
                assert!(
                    full.volume >= Decimal::ZERO,
                    "Volume should be non-negative"
                );
                assert!(
                    full.quote_volume >= Decimal::ZERO,
                ...
                assert!(
                    full.close_time >= full.open_time,
            ...
            TickerStatistics::Mini(mini) => {
                ...
                assert!(
                    mini.last_price > Decimal::ZERO,
                    "Last price should be positive"
                );
                assert!(
                    mini.volume >= Decimal::ZERO,
                    "Volume should be non-negative"
                );
                assert!(
                    mini.quote_volume >= Decimal::ZERO,
                    "Quote volume should be non-negative"
                );
                assert!(mini.open_time > 0, "Open time should be positive");
                assert!(mini.close_time > 0, "Close time should be positive");
                assert!(
                    mini.close_time >= mini.open_time,
                    "Close time should be >= open time"
                );
            }
        }
    ...
    async fn test_ticker_price_all_symbols() {
        ...
        let rest_prices = with_timeout(rest_client.ticker_price(rest_spec))
            .await
            .expect("REST ticker price");

        let ws_spec = TickerPriceSpec::new().build().expect("Spec validation");
        let ws_prices = with_timeout(ws_client.ticker_price(ws_spec))
            .await
            .expect("WebSocket ticker price");

        // Assert
        assert!(
            !rest_prices.is_empty(),
            "REST should return at least one price"
        );
        assert!(
            !ws_prices.is_empty(),
            "WebSocket should return at least one price"
        );
    ...
    async fn test_ticker_book_all_symbols() {
        ...
        let rest_books = with_timeout(rest_client.ticker_book(rest_spec))
            .await
            .expect("REST ticker book");

        let ws_spec = TickerBookSpec::new().build().expect("Spec validation");
        let ws_books = with_timeout(ws_client.ticker_book(ws_spec))
            .await
            .expect("WebSocket ticker book");

        // Assert
        assert!(
            !rest_books.is_empty(),
            "REST should return at least one book ticker"
        );
        assert!(
            !ws_books.is_empty(),
            "WebSocket should return at least one book ticker"
        );
        ...
        assert_eq!(
            rest_books.len(),
            ws_books.len(),
            "Book ticker count should match"
        );
    }
    ...
    async fn test_ticker_book_multiple_symbols() {
        ...
        assert_eq!(
            rest_books.len(),
            2,
            "Should return exactly two book tickers"
        );
    ...
    async fn test_ticker_24hr_specific_symbol() {
        ...
        let rest_spec = Ticker24HrSpec::new()
            ...
            .build()
            .expect("Spec validation");
        let rest_stats = with_timeout(rest_client.ticker_24hr(rest_spec))
            .await
            .expect("REST 24hr ticker");

        let ws_spec = Ticker24HrSpec::new()
            .with_symbol(test_symbol)
            .build()
            .expect("Spec validation");
        let ws_stats = with_timeout(ws_client.ticker_24hr(ws_spec))
            .await
            .expect("WebSocket 24hr ticker");

        // Assert
        assert_eq!(
            rest_stats.len(),
            1,
            "Should return exactly one ticker statistic"
        );
        assert_eq!(
            ws_stats.len(),
            1,
    ...
    async fn test_ticker_rolling_window_multiple_symbols() {
        ...
        let rest_spec = TickerRollingWindowSpec::new()
            ...
            .build()
            .expect("Spec validation");
        let rest_stats = with_timeout(rest_client.ticker_rolling_window(rest_spec))
            .await
            .expect("REST rolling window ticker");

        let ws_spec = TickerRollingWindowSpec::new()
            .with_symbols(test_symbols.clone())
            .build()
            .expect("Spec validation");
        let ws_stats = with_timeout(ws_client.ticker_rolling_window(ws_spec))
            .await
            .expect("WebSocket rolling window ticker");

        // Assert
        assert_eq!(
            rest_stats.len(),
            2,
            "Should return exactly two ticker statistics"
        );
        assert_eq!(
            ws_stats.len(),
            2,
        ...
        let rest_symbols: Vec<String> = rest_stats
            .iter()
            .map(|s| match s {
        ...
        let ws_symbols: Vec<String> = ws_stats
            .iter()
            .map(|s| match s {
                TickerStatistics::Full(full) => full.symbol.clone(),
                TickerStatistics::Mini(mini) => mini.symbol.clone(),
    ...
    async fn test_ticker_trading_day_multiple_symbols() {
        ...
        let rest_spec = TickerTradingDaySpec::new()
            ...
            .build()
            .expect("Spec validation");
        let rest_stats = with_timeout(rest_client.ticker_trading_day(rest_spec))
            .await
            .expect("REST trading day ticker");

        let ws_spec = TickerTradingDaySpec::new()
            .with_symbols(test_symbols.clone())
            .build()
            .expect("Spec validation");
        let ws_stats = with_timeout(ws_client.ticker_trading_day(ws_spec))
            .await
            .expect("WebSocket trading day ticker");
    ...
    async fn test_ticker_24hr_all_symbols() {
        ...
        assert!(
            !rest_stats.is_empty(),
            "REST should return at least one ticker statistic"
        );
        assert!(
            !ws_stats.is_empty(),
            "WebSocket should return at least one ticker statistic"
        );
        ...
        assert_eq!(
            rest_stats.len(),
            ws_stats.len(),
            "Ticker statistics count should match"
        );
    }
    ...
    async fn test_ticker_price_invalid_symbol() {
        ...
        match ws_result {
            Ok(prices) => panic!(
                "Expected WebSocket error, got successful response: {:?}",
                prices
            ),
            Err(err) => {
                let downcast = err.downcast_ref::<BinanceError>();
