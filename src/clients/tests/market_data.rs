...
mod tests {
    ...
    fn assert_valid_order_book(order_book: &OrderBook) {
        ...
        assert!(order_book.last_update_id > 0, "Last update ID should be positive");
        ...
        if order_book.bids.len() > 1 {
            assert!(order_book.bids[0].price >= order_book.bids[1].price, "Bids should be in descending price order");
        }
        if order_book.asks.len() > 1 {
            assert!(order_book.asks[0].price <= order_book.asks[1].price, "Asks should be in ascending price order");
        }
    }
    ...
    fn assert_valid_trade(trade: &Trade) {
        ...
        assert!(trade.price > Decimal::ZERO, "Trade price should be positive");
        assert!(trade.quantity > Decimal::ZERO, "Trade quantity should be positive");
        if trade.quote_quantity.is_some() {
            assert!(trade.quote_quantity > Some(Decimal::ZERO), "Quote quantity should be positive");
        }
    ...
    fn assert_valid_aggregate_trade(agg_trade: &AggregateTrade) {
        ...
        assert!(agg_trade.quantity > Decimal::ZERO, "Quantity should be positive");
        assert!(agg_trade.first_trade_id > 0, "First trade ID should be positive");
        assert!(agg_trade.last_trade_id > 0, "Last trade ID should be positive");
        assert!(agg_trade.last_trade_id >= agg_trade.first_trade_id, "Last trade ID should be >= first trade ID");
    ...
    fn assert_valid_kline(kline: &Kline) {
        ...
        assert!(kline.close_time > kline.open_time, "Close time should be after open time");
        assert!(kline.open_price > Decimal::ZERO, "Open price should be positive");
        assert!(kline.high_price > Decimal::ZERO, "High price should be positive");
        assert!(kline.low_price > Decimal::ZERO, "Low price should be positive");
        assert!(kline.close_price > Decimal::ZERO, "Close price should be positive");
        assert!(kline.volume >= Decimal::ZERO, "Volume should be non-negative");
        assert!(kline.quote_asset_volume >= Decimal::ZERO, "Quote asset volume should be non-negative");
        assert!(kline.taker_buy_base_asset_volume >= Decimal::ZERO, "Taker buy base volume should be non-negative");
        assert!(kline.taker_buy_quote_asset_volume >= Decimal::ZERO, "Taker buy quote volume should be non-negative");
    ...
    fn assert_valid_average_price(avg_price: &AveragePrice) {
        ...
        assert!(avg_price.price > Decimal::ZERO, "Average price should be positive");
    ...
    async fn test_order_book_default_limit() {
        ...
        // Act
        let rest_spec = OrderBookSpec::new(test_symbol).build().expect("Spec validation");
        let rest_order_book = with_timeout(rest_client.order_book(rest_spec)).await.expect("REST order book");
        
        let ws_spec = OrderBookSpec::new(test_symbol).build().expect("Spec validation");
        let ws_order_book = with_timeout(ws_client.order_book(ws_spec)).await.expect("WebSocket order book");
        ...
        assert!(ws_order_book.asks.len() <= 100, "Asks should not exceed default limit");
    }
    ...
    async fn test_order_book_custom_limit() {
        ...
        // Act
        let rest_spec = OrderBookSpec::new(test_symbol).with_limit(limit).build().expect("Spec validation");
        let rest_order_book = with_timeout(rest_client.order_book(rest_spec)).await.expect("REST order book");
        
        let ws_spec = OrderBookSpec::new(test_symbol).with_limit(limit).build().expect("Spec validation");
        let ws_order_book = with_timeout(ws_client.order_book(ws_spec)).await.expect("WebSocket order book");
        ...
        assert!(rest_order_book.bids.len() <= limit as usize, "Bids should not exceed custom limit");
        assert!(rest_order_book.asks.len() <= limit as usize, "Asks should not exceed custom limit");
        assert!(ws_order_book.bids.len() <= limit as usize, "Bids should not exceed custom limit");
        assert!(ws_order_book.asks.len() <= limit as usize, "Asks should not exceed custom limit");
    }
    ...
    async fn test_order_book_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(order_book) => panic!("Expected {} error, got successful response: {:?}", client_name, order_book),
                Err(err) => {
    ...
    async fn test_recent_trades_default_limit() {
        ...
        // Act
        let rest_spec = RecentTradesSpec::new(test_symbol).build().expect("Spec validation");
        let rest_trades = with_timeout(rest_client.recent_trades(rest_spec)).await.expect("REST recent trades");
        
        let ws_spec = RecentTradesSpec::new(test_symbol).build().expect("Spec validation");
        let ws_trades = with_timeout(ws_client.recent_trades(ws_spec)).await.expect("WebSocket recent trades");
        
        // Assert
        assert!(!rest_trades.is_empty(), "REST should return recent trades");
        assert!(!ws_trades.is_empty(), "WebSocket should return recent trades");
    ...
    async fn test_recent_trades_custom_limit() {
        ...
        // Act
        let rest_spec = RecentTradesSpec::new(test_symbol).with_limit(limit).build().expect("Spec validation");
        let rest_trades = with_timeout(rest_client.recent_trades(rest_spec)).await.expect("REST recent trades");
        
        let ws_spec = RecentTradesSpec::new(test_symbol).with_limit(limit).build().expect("Spec validation");
        let ws_trades = with_timeout(ws_client.recent_trades(ws_spec)).await.expect("WebSocket recent trades");
        ...
        assert!(rest_trades.len() <= limit as usize, "REST trades should not exceed custom limit");
        assert!(ws_trades.len() <= limit as usize, "WebSocket trades should not exceed custom limit");
    }
    ...
    async fn test_historical_trades_default_limit() {
        ...
        // Act
        let rest_spec = HistoricalTradesSpec::new(test_symbol).build().expect("Spec validation");
        let rest_trades = with_timeout(rest_client.historical_trades(rest_spec)).await.expect("REST historical trades");
        
        let ws_spec = HistoricalTradesSpec::new(test_symbol).build().expect("Spec validation");
        let ws_trades = with_timeout(ws_client.historical_trades(ws_spec)).await.expect("WebSocket historical trades");
        
        // Assert
        assert!(!rest_trades.is_empty(), "REST should return historical trades");
        assert!(!ws_trades.is_empty(), "WebSocket should return historical trades");
    ...
    async fn test_historical_trades_from_id() {
        ...
        let ws_spec = HistoricalTradesSpec::new(test_symbol).with_from_id(from_id).build().expect("Spec validation");
        let ws_trades = with_timeout(ws_client.historical_trades(ws_spec)).await.expect("WebSocket historical trades");
        ...
        for trade in &rest_trades {
            assert_valid_trade(trade);
            assert!(trade.id >= from_id, "Trade ID should be >= from_id parameter");
        }
    ...
    async fn test_aggregate_trades_default_limit() {
        ...
        // Act
        let rest_spec = AggregateTradesSpec::new(test_symbol).build().expect("Spec validation");
        let rest_agg_trades = with_timeout(rest_client.aggregate_trades(rest_spec)).await.expect("REST aggregate trades");
        
        let ws_spec = AggregateTradesSpec::new(test_symbol).build().expect("Spec validation");
        let ws_agg_trades = with_timeout(ws_client.aggregate_trades(ws_spec)).await.expect("WebSocket aggregate trades");
        
        // Assert
        assert!(!rest_agg_trades.is_empty(), "REST should return aggregate trades");
        assert!(!ws_agg_trades.is_empty(), "WebSocket should return aggregate trades");
    ...
    async fn test_aggregate_trades_time_range() {
        ...
        // Assert
        assert!(!rest_agg_trades.is_empty(), "REST should return aggregate trades in time range");
        assert!(!ws_agg_trades.is_empty(), "WebSocket should return aggregate trades in time range");
        
        for agg_trade in &rest_agg_trades {
            assert_valid_aggregate_trade(agg_trade);
            assert!(agg_trade.timestamp >= start_time, "Trade timestamp should be >= start_time");
            assert!(agg_trade.timestamp <= end_time, "Trade timestamp should be <= end_time");
        }
    ...
    async fn test_klines_default_params() {
        ...
        // Act
        let rest_spec = KlinesSpec::new(test_symbol, interval).build().expect("Spec validation");
        let rest_klines = with_timeout(rest_client.klines(rest_spec)).await.expect("REST klines");
        
        let ws_spec = KlinesSpec::new(test_symbol, interval).build().expect("Spec validation");
        let ws_klines = with_timeout(ws_client.klines(ws_spec)).await.expect("WebSocket klines");
    ...
    async fn test_klines_custom_params() {
        ...
        for kline in &rest_klines {
            assert_valid_kline(kline);
            assert!(kline.open_time >= start_time, "Kline open time should be >= start_time");
            assert!(kline.close_time <= end_time, "Kline close time should be <= end_time");
        }
        ...
        assert!(rest_klines.len() <= limit as usize, "REST klines should not exceed custom limit");
        assert!(ws_klines.len() <= limit as usize, "WebSocket klines should not exceed custom limit");
    }
    ...
    async fn test_klines_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(klines) => panic!("Expected {} error, got successful response: {:?}", client_name, klines),
                Err(err) => {
                    let downcast = err.downcast_ref::<BinanceError>();
                    assert!(
                        matches!(
    ...
    async fn test_ui_klines_default_params() {
        ...
        // Act
        let rest_spec = KlinesSpec::new(test_symbol, interval).build().expect("Spec validation");
        let rest_ui_klines = with_timeout(rest_client.ui_klines(rest_spec)).await.expect("REST UI klines");
        
        let ws_spec = KlinesSpec::new(test_symbol, interval).build().expect("Spec validation");
        let ws_ui_klines = with_timeout(ws_client.ui_klines(ws_spec)).await.expect("WebSocket UI klines");
        
        // Assert
        assert!(!rest_ui_klines.is_empty(), "REST should return UI klines");
        assert!(!ws_ui_klines.is_empty(), "WebSocket should return UI klines");
    ...
    async fn test_ui_klines_with_timezone() {
        ...
        // Assert
        assert!(!rest_ui_klines.is_empty(), "REST should return UI klines with timezone");
        assert!(!ws_ui_klines.is_empty(), "WebSocket should return UI klines with timezone");
        ...
        assert!(rest_ui_klines.len() <= limit as usize, "REST UI klines should not exceed custom limit");
        assert!(ws_ui_klines.len() <= limit as usize, "WebSocket UI klines should not exceed custom limit");
    }
    ...
    async fn test_average_price() {
        ...
        // Act
        let rest_spec = AveragePriceSpec::new(test_symbol).build().expect("Spec validation");
        let rest_avg_price = with_timeout(rest_client.average_price(rest_spec)).await.expect("REST average price");
        
        let ws_spec = AveragePriceSpec::new(test_symbol).build().expect("Spec validation");
        let ws_avg_price = with_timeout(ws_client.average_price(ws_spec)).await.expect("WebSocket average price");
    ...
    async fn test_average_price_invalid_symbol() {
        ...
        for (client_name, result) in [("REST", rest_result), ("WebSocket", ws_result)] {
            match result {
                Ok(avg_price) => panic!("Expected {} error, got successful response: {:?}", client_name, avg_price),
                Err(err) => {
                    let downcast = err.downcast_ref::<BinanceError>();
                    assert!(
                        matches!(
