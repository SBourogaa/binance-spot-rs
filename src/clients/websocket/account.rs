...
use crate::Result;
use crate::{
    clients::{r#trait::AccountClient, websocket::BinanceSpotWebSocketClient},
    types::{
        requests::{
...
impl AccountClient for BinanceSpotWebSocketClient {
    ...
    async fn commission_rates(
        ...
        self.signed_request("account.commission", specification)
            .await
    }
    ...
    async fn open_orders(&self, specification: OpenOrdersSpec<Validated>) -> Result<Vec<Order>> {
        self.signed_request("openOrders.status", specification)
            .await
    }
    ...
    async fn prevented_matches(
        ...
        self.signed_request("myPreventedMatches", specification)
            .await
    }
