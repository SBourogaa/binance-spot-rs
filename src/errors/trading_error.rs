...
impl TradingError {
    ...
    pub fn is_order_placement(&self) -> bool {
        matches!(self, Self::NewOrderRejected)
    }
    ...
    pub fn is_order_cancellation(&self) -> bool {
        matches!(
            self,
            Self::CancelRejected | Self::CancelReplacePartiallyFailed | Self::CancelReplaceFailed
        )
    }
    ...
    pub fn is_auth_related(&self) -> bool {
        matches!(self, Self::BadApiKeyFormat | Self::RejectedMbxKey)
    }
    ...
    pub fn is_order_not_found(&self) -> bool {
        matches!(
            self,
            Self::NoSuchOrder | Self::OrderArchived | Self::ClientOrderIdInvalid
        )
    }
    ...
    pub fn is_stream_related(&self) -> bool {
        matches!(self, Self::SubscriptionActive | Self::SubscriptionInactive)
    }
    ...
    pub fn user_message(&self) -> &'static str {
        match self {
            Self::NewOrderRejected => {
                "Order was rejected - check order parameters and account status"
            }
            Self::CancelRejected => {
                "Order cancellation was rejected - order may already be filled or canceled"
            }
            ...
            Self::CancelReplacePartiallyFailed => {
                "Cancel-replace partially failed - check order status"
            }
    ...
    pub fn suggested_action(&self) -> &'static str {
        match self {
            ...
            Self::CancelReplacePartiallyFailed => {
                "Check which operation failed and retry if needed"
            }
            ...
            Self::SubscriptionActive => {
                "Use existing subscription or close before creating new one"
            }
