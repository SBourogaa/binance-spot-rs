...
mod tests {
    ...
    use crate::streams::{BinanceSpotStreamClient, events::*, specs::*};
    ...
    async fn with_recv_timeout<T>(
        future: impl std::future::Future<
            Output = std::result::Result<T, tokio::sync::broadcast::error::RecvError>,
        >,
    ) -> Result<T> {
        tokio::time::timeout(Duration::from_secs(30), future)
            .await
    ...
    fn assert_valid_aggregate_trade_event(event: &AggregateTradeStreamEvent) {
        ...
        assert_eq!(
            event.event_type, "aggTrade",
            "Event type should be aggTrade"
        );
        assert!(event.event_time > 0, "Event time should be positive");
        assert!(
            event.aggregate_trade.id > 0,
            "Aggregate trade ID should be positive"
        );
        assert!(
            event.aggregate_trade.price > Decimal::ZERO,
            "Price should be positive"
        );
        assert!(
            event.aggregate_trade.quantity > Decimal::ZERO,
            "Quantity should be positive"
        );
        assert!(
            event.aggregate_trade.first_trade_id > 0,
            "First trade ID should be positive"
        );
        assert!(
            event.aggregate_trade.last_trade_id > 0,
            "Last trade ID should be positive"
        );
        assert!(
            event.aggregate_trade.last_trade_id >= event.aggregate_trade.first_trade_id,
            "Last trade ID should be >= first trade ID"
        );
        assert!(
            event.aggregate_trade.timestamp > 0,
    ...
    fn assert_valid_trade_event(event: &TradeStreamEvent) {
        ...
        assert!(
            event.trade.price > Decimal::ZERO,
            "Price should be positive"
        );
        assert!(
            event.trade.quantity > Decimal::ZERO,
            "Quantity should be positive"
        );
    ...
    fn assert_valid_book_ticker_event(event: &BookTickerStreamEvent) {
        assert!(
            !event.ticker.symbol.is_empty(),
            "Symbol should not be empty"
        );
        assert!(
            event.ticker.bid_price > Decimal::ZERO,
            "Bid price should be positive"
        );
        assert!(
            event.ticker.ask_price > Decimal::ZERO,
            "Ask price should be positive"
        );
        assert!(
            event.ticker.bid_quantity > Decimal::ZERO,
            "Bid quantity should be positive"
        );
        assert!(
            event.ticker.ask_quantity > Decimal::ZERO,
            "Ask quantity should be positive"
        );
        assert!(
            event.ticker.ask_price >= event.ticker.bid_price,
    ...
    fn assert_valid_mini_ticker_event(event: &MiniTickerStreamEvent) {
        assert!(!event.symbol.is_empty(), "Symbol should not be empty");
        assert_eq!(
            event.event_type, "24hrMiniTicker",
            "Event type should be 24hrMiniTicker"
        );
        assert!(event.event_time > 0, "Event time should be positive");
        assert!(
            event.close_price > Decimal::ZERO,
            "Close price should be positive"
        );
        assert!(
            event.open_price > Decimal::ZERO,
            "Open price should be positive"
        );
        assert!(
            event.high_price > Decimal::ZERO,
            "High price should be positive"
        );
        assert!(
            event.low_price > Decimal::ZERO,
        ...
        assert!(
            event.quote_volume >= Decimal::ZERO,
    ...
    fn assert_valid_ticker_event(event: &TickerStreamEvent) {
        ...
        assert_eq!(
            event.event_type, "24hrTicker",
            "Event type should be 24hrTicker"
        );
        assert!(event.event_time > 0, "Event time should be positive");
        assert!(
            event.ticker.last_price > Decimal::ZERO,
            "Last price should be positive"
        );
        assert!(
            event.ticker.volume >= Decimal::ZERO,
            "Volume should be non-negative"
        );
        assert!(
            event.ticker.quote_volume >= Decimal::ZERO,
            "Quote volume should be non-negative"
        );
        ...
        assert!(
            event.ticker.close_time >= event.ticker.open_time,
            "Close time should be >= open time"
        );
    }
    ...
    fn assert_valid_kline_event(event: &KlineStreamEvent) {
        ...
        assert!(event.event_time > 0, "Event time should be positive");
        assert!(
            !event.kline.interval.is_empty(),
            "Interval should not be empty"
        );
        assert!(
            event.kline.kline.open_time > 0,
            "Open time should be positive"
        );
        assert!(
            event.kline.kline.close_time > 0,
            "Close time should be positive"
        );
        assert!(
            event.kline.kline.close_time > event.kline.kline.open_time,
            "Close time should be after open time"
        );
        assert!(
            event.kline.kline.open_price > Decimal::ZERO,
            "Open price should be positive"
        );
        assert!(
            event.kline.kline.high_price > Decimal::ZERO,
            "High price should be positive"
        );
        assert!(
            event.kline.kline.low_price > Decimal::ZERO,
            "Low price should be positive"
        );
        assert!(
            event.kline.kline.close_price > Decimal::ZERO,
            "Close price should be positive"
        );
        assert!(
            event.kline.kline.volume >= Decimal::ZERO,
            "Volume should be non-negative"
        );
        assert!(
            event.kline.kline.quote_asset_volume >= Decimal::ZERO,
    ...
    fn assert_valid_diff_depth_event(event: &DiffDepthStreamEvent) {
        assert!(!event.symbol.is_empty(), "Symbol should not be empty");
        assert_eq!(
            event.event_type, "depthUpdate",
            "Event type should be depthUpdate"
        );
        assert!(event.event_time > 0, "Event time should be positive");
        assert!(
            event.first_update_id > 0,
            "First update ID should be positive"
        );
        assert!(
            event.final_update_id > 0,
            "Final update ID should be positive"
        );
        assert!(
        ...
        for bid in &event.bids {
            ...
            assert!(
                bid.quantity() >= Decimal::ZERO,
                "Bid quantity should be non-negative (0 means remove)"
            );
        }
        ...
        for ask in &event.asks {
            ...
            assert!(
                ask.quantity() >= Decimal::ZERO,
                "Ask quantity should be non-negative (0 means remove)"
            );
        }
    }
    ...
    fn assert_valid_all_mini_tickers_event(event: &AllMiniTickersStreamEvent) {
        ...
        for ticker in &event.tickers[..3.min(event.tickers.len())] {
            ...
            assert_eq!(
                ticker.event_type, "24hrMiniTicker",
                "Event type should be 24hrMiniTicker"
            );
            assert!(ticker.event_time > 0, "Event time should be positive");
            assert!(
                ticker.close_price > Decimal::ZERO,
                "Close price should be positive"
            );
            assert!(
                ticker.open_price > Decimal::ZERO,
                "Open price should be positive"
            );
            assert!(
                ticker.volume >= Decimal::ZERO,
                "Volume should be non-negative"
            );
        }
    }
    ...
    fn assert_valid_all_tickers_event(event: &AllTickersStreamEvent) {
        ...
        for ticker in &event.tickers[..3.min(event.tickers.len())] {
            assert!(
                !ticker.ticker.symbol.is_empty(),
                "Symbol should not be empty"
            );
            assert_eq!(
                ticker.event_type, "24hrTicker",
                "Event type should be 24hrTicker"
            );
            assert!(ticker.event_time > 0, "Event time should be positive");
            assert!(
                ticker.ticker.last_price > Decimal::ZERO,
                "Last price should be positive"
            );
            assert!(
                ticker.ticker.volume >= Decimal::ZERO,
                "Volume should be non-negative"
            );
    ...
    async fn test_aggregate_trade_stream_raw() {
        ...
        let mut subscription = with_timeout(client.subscribe(&spec))
            .await
            .expect("Subscription");
        let event = with_recv_timeout(subscription.recv())
            .await
    ...
    async fn test_aggregate_trade_stream_combined() {
        ...
        let mut first_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("First subscription");
        let first_event = with_recv_timeout(first_subscription.recv())
            .await
            .expect("Receive first event");

        let mut second_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("Second subscription");
        let second_event = with_recv_timeout(second_subscription.recv())
            .await
    ...
    async fn test_aggregate_trade_stream_dynamic() {
        ...
        let mut agg_trade_subscription = with_timeout(client.subscribe(&agg_trade_spec))
            .await
            .expect("Aggregate trade subscription");
        let agg_trade_event = with_recv_timeout(agg_trade_subscription.recv())
            .await
            .expect("Receive aggregate trade event");

        let trade_spec = TradeStreamSpec::new(trade_symbol);
        let mut trade_subscription = with_timeout(client.subscribe(&trade_spec))
            .await
            .expect("Trade subscription");
        let trade_event = with_recv_timeout(trade_subscription.recv())
            .await
            .expect("Receive trade event");

        let book_ticker_spec = BookTickerStreamSpec::new(book_ticker_symbol);
        let mut book_ticker_subscription = with_timeout(client.subscribe(&book_ticker_spec))
            .await
            .expect("Book ticker subscription");
        let book_ticker_event = with_recv_timeout(book_ticker_subscription.recv())
            .await
            .expect("Receive book ticker event");
    ...
    async fn test_all_mini_tickers_stream_combined() {
        ...
        let mut all_mini_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("All mini tickers subscription");
        let all_mini_event = with_recv_timeout(all_mini_subscription.recv())
            .await
            .expect("Receive all mini tickers event");
    ...
    async fn test_all_mini_tickers_stream_dynamic() {
        ...
        let mut all_mini_subscription = with_timeout(client.subscribe(&all_mini_spec))
            .await
            .expect("All mini tickers subscription");
        let all_mini_event = with_recv_timeout(all_mini_subscription.recv())
            .await
        ...
        let mut kline_subscription = with_timeout(client.subscribe(&kline_spec))
            .await
            .expect("Kline subscription");
        let kline_event = with_recv_timeout(kline_subscription.recv())
            .await
            .expect("Receive kline event");
    ...
    async fn test_all_rolling_window_tickers_stream_combined() {
        ...
        let mut hourly_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("Hourly all tickers subscription");
        let hourly_event = with_recv_timeout(hourly_subscription.recv())
            .await
            .expect("Receive hourly event");

        let mut four_hourly_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("4-hourly all tickers subscription");
        let four_hourly_event = with_recv_timeout(four_hourly_subscription.recv())
            .await
            .expect("Receive 4-hourly event");
        ...
        assert!(
            hourly_event.tickers.len() > 10,
            "Should have many hourly tickers"
        );

        assert_valid_all_rolling_window_tickers_event(&four_hourly_event);
        assert!(
            four_hourly_event.tickers.len() > 10,
            "Should have many 4-hourly tickers"
        );
    }
    ...
    async fn test_all_rolling_window_tickers_stream_dynamic() {
        ...
        let mut all_rolling_subscription = with_timeout(client.subscribe(&all_rolling_spec))
            .await
            .expect("All rolling window tickers subscription");
        let all_rolling_event = with_recv_timeout(all_rolling_subscription.recv())
            .await
            .expect("Receive all rolling window event");
        ...
        assert!(
            all_rolling_event.tickers.len() > 10,
            "Should have many rolling window tickers"
        );
    ...
    async fn test_all_tickers_stream_combined() {
        ...
        let mut btc_ticker_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC ticker subscription");
        let btc_ticker_event = with_recv_timeout(btc_ticker_subscription.recv())
            .await
            .expect("Receive BTC ticker event");

        let mut eth_ticker_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH ticker subscription");
        let eth_ticker_event = with_recv_timeout(eth_ticker_subscription.recv())
            .await
            .expect("Receive ETH ticker event");
    ...
    async fn test_all_tickers_stream_dynamic() {
        ...
        let mut all_tickers_subscription = with_timeout(client.subscribe(&all_tickers_spec))
            .await
            .expect("All tickers subscription");
        let all_tickers_event = with_recv_timeout(all_tickers_subscription.recv())
            .await
            .expect("Receive all tickers event");
        ...
        let mut mini_ticker_subscription = with_timeout(client.subscribe(&mini_ticker_spec))
            .await
            .expect("Mini ticker subscription");
        let mini_ticker_event = with_recv_timeout(mini_ticker_subscription.recv())
            .await
            .expect("Receive mini ticker event");
        ...
        assert!(
            all_tickers_event.tickers.len() > 10,
            "Should have many tickers"
        );
    ...
    async fn test_average_price_stream_combined() {
        ...
        let mut btc_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC average price subscription");
        let btc_event = with_recv_timeout(btc_subscription.recv())
            .await
            .expect("Receive BTC event");

        let mut eth_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH average price subscription");
        let eth_event = with_recv_timeout(eth_subscription.recv())
            .await
            .expect("Receive ETH event");
    ...
    async fn test_average_price_stream_dynamic() {
        ...
        let mut avg_price_subscription = with_timeout(client.subscribe(&avg_price_spec))
            .await
            .expect("Average price subscription");
        let avg_price_event = with_recv_timeout(avg_price_subscription.recv())
            .await
            .expect("Receive average price event");
        ...
        let mut ticker_subscription = with_timeout(client.subscribe(&ticker_spec))
            .await
            .expect("Ticker subscription");
        let ticker_event = with_recv_timeout(ticker_subscription.recv())
            .await
            .expect("Receive ticker event");
    ...
    async fn test_book_ticker_stream_combined() {
        ...
        let mut btc_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC book ticker subscription");
        let btc_event = with_recv_timeout(btc_subscription.recv())
            .await
            .expect("Receive BTC event");
        ...
        let mut eth_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH book ticker subscription");
        let eth_event = with_recv_timeout(eth_subscription.recv())
            .await
            .expect("Receive ETH event");
    ...
    async fn test_diff_depth_stream_combined() {
        ...
        let mut btc_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC diff depth subscription");
        let btc_event = with_recv_timeout(btc_subscription.recv())
            .await
            .expect("Receive BTC event");
        ...
        let mut eth_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH diff depth subscription");
        let eth_event = with_recv_timeout(eth_subscription.recv())
            .await
            .expect("Receive ETH event");
    ...
    async fn test_diff_depth_stream_dynamic() {
        ...
        let mut diff_depth_subscription = with_timeout(client.subscribe(&diff_depth_spec))
            .await
            .expect("Diff depth subscription");
        let diff_depth_event = with_recv_timeout(diff_depth_subscription.recv())
            .await
            .expect("Receive diff depth event");
    ...
    async fn test_kline_with_timezone_stream_dynamic() {
        ...
        let mut kline_tz_subscription = with_timeout(client.subscribe(&kline_tz_spec))
            .await
            .expect("Kline timezone subscription");
        let kline_tz_event = with_recv_timeout(kline_tz_subscription.recv())
            .await
            .expect("Receive kline timezone event");
    ...
    async fn test_mini_ticker_stream_combined() {
        ...
        let mut btc_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC mini ticker subscription");
        let btc_event = with_recv_timeout(btc_subscription.recv())
            .await
            .expect("Receive BTC event");
        ...
        let mut eth_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH mini ticker subscription");
        let eth_event = with_recv_timeout(eth_subscription.recv())
            .await
            .expect("Receive ETH event");
    ...
    async fn test_partial_book_depth_stream_combined() {
        ...
        let mut btc_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC partial depth subscription");
        let btc_event = with_recv_timeout(btc_subscription.recv())
            .await
            .expect("Receive BTC event");
        ...
        let mut eth_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH partial depth subscription");
        let eth_event = with_recv_timeout(eth_subscription.recv())
            .await
            .expect("Receive ETH event");
    ...
    async fn test_partial_book_depth_stream_dynamic() {
        ...
        let mut partial_depth_subscription = with_timeout(client.subscribe(&partial_depth_spec))
            .await
            .expect("Partial depth subscription");
        let partial_depth_event = with_recv_timeout(partial_depth_subscription.recv())
            .await
            .expect("Receive partial depth event");
        ...
        assert!(
            partial_depth_event.bids.len() <= 20,
            "Should have at most 20 bids"
        );
        assert!(
            partial_depth_event.asks.len() <= 20,
            "Should have at most 20 asks"
        );
    ...
    async fn test_rolling_window_ticker_stream_combined() {
        ...
        let mut btc_subscription = with_timeout(client.subscribe(&first_spec))
            .await
            .expect("BTC rolling window subscription");
        let btc_event = with_recv_timeout(btc_subscription.recv())
            .await
            .expect("Receive BTC event");
        ...
        let mut eth_subscription = with_timeout(client.subscribe(&second_spec))
            .await
            .expect("ETH rolling window subscription");
        let eth_event = with_recv_timeout(eth_subscription.recv())
            .await
            .expect("Receive ETH event");
    ...
    async fn test_rolling_window_ticker_stream_dynamic() {
        ...
        let mut rolling_window_subscription = with_timeout(client.subscribe(&rolling_window_spec))
            .await
            .expect("Rolling window ticker subscription");
        let rolling_window_event = with_recv_timeout(rolling_window_subscription.recv())
            .await
            .expect("Receive rolling window event");
    ...
    async fn test_ticker_stream_dynamic() {
        ...
        assert_valid_partial_book_depth_event(&partial_depth_event);
        assert!(
            partial_depth_event.bids.len() <= 10,
            "Should have at most 10 bids"
        );
        assert!(
            partial_depth_event.asks.len() <= 10,
            "Should have at most 10 asks"
        );
    ...
    async fn test_trade_stream_dynamic() {
        ...
        let mut rolling_window_subscription = with_timeout(client.subscribe(&rolling_window_spec))
            .await
            .expect("Rolling window subscription");
        let rolling_window_event = with_recv_timeout(rolling_window_subscription.recv())
            .await
            .expect("Receive rolling window event");
    ...
    async fn test_connection_status_max_retries_exceeded() {
        ...
        let client = crate::streams::client(config).expect("Client creation");
