...
mod tests {
    ...
    use crate::clients::r#trait::{BinanceSpotClient, GeneralClient, TickerClient, TradingClient};
    ...
    use crate::types::requests::{CancelOrderSpec, ExchangeInfoSpec, OrderSpec, TickerPriceSpec};
    use crate::types::responses::SymbolInfo;
    use crate::{BinanceConfig, RestConfig, StreamConfig, WebSocketConfig};
    ...
    fn create_user_data_stream_client() -> Result<BinanceSpotStreamClient> {
        let config = BinanceConfig::<StreamConfig>::builder()
            ...
            .with_credentials_from_file(
                ...
                std::env::var("BINANCE_TESTNET_PEM_FILE")
                    .expect("BINANCE_TESTNET_PEM_FILE required"),
            )?
            .with_user_data()
            .build()?;
    ...
    fn assert_valid_execution_report_event(event: &ExecutionReportEvent) {
        ...
        assert!(
            !event.client_order_id.is_empty(),
        ...
        assert!(
            event.last_executed_quantity >= Decimal::ZERO,
            "Last executed quantity should be non-negative"
        );
        assert!(
            event.cumulative_quote_quantity >= Decimal::ZERO,
            "Cumulative quote quantity should be non-negative"
        );
        assert!(
            event.transaction_time > 0,
    ...
    fn assert_valid_account_position_event(event: &OutboundAccountPositionEvent) {
        assert!(event.event_time > 0, "Event time should be positive");
        assert!(
            event.last_update_time > 0,
            "Last update time should be positive"
        );
        assert!(
            !event.balances.is_empty(),
    ...
    async fn wait_for_execution_report(
        ...
        Err(anyhow::anyhow!(
            "ExecutionReport not received within {} attempts",
            max_attempts
        ))
    }
    ...
    async fn test_user_data_stream_connection() {
        ...
        assert!(
            ...
            "Connection should succeed with valid credentials"
        );
    ...
    async fn test_execution_report_via_rest_order() {
        ...
        let (safe_price, safe_quantity) = get_safe_btcusdt_order_params()
            .await
        ...
        let order_result = rest_client
            .place_order(order_spec)
            .await
            .expect("Order placement");

        let execution_report = wait_for_execution_report(&mut subscription, 5)
            .await
            .expect("Receive execution report");
        ...
        let _ = rest_client
            .cancel_order(cancel_spec)
            .await
            .expect("Order cancellation");

        let cancel_execution_report = wait_for_execution_report(&mut subscription, 5)
            .await
            .expect("Receive cancel execution report");
        ...
        assert!(matches!(
            cancel_execution_report.order_status,
            crate::enums::OrderStatus::Canceled
        ));
    ...
    async fn test_execution_report_via_websocket_order() {
        // Arrange
        let mut stream_client = create_user_data_stream_client().expect("Stream client creation");
        let ws_client = create_websocket_client()
            .await
            .expect("WebSocket client creation");
        let spec = UserDataStreamSpec::new();
        ...
        let _test_order_result = ws_client
            .test_order(order_spec)
            .await
            .expect("Test order placement");
    ...
    async fn test_account_position_events() {
        ...
        // Assert
        assert!(
            received_execution_report,
            "Should receive at least one execution report"
        );
    ...
    async fn test_multiple_user_data_subscriptions() {
        ...
        let _subscription1 = with_timeout(client.subscribe(&spec))
            .await
            .expect("First subscription");
        let _subscription2 = with_timeout(client.subscribe(&spec))
            .await
    ...
    async fn test_connection_status_lifecycle() {
        ...
        assert!(
            matches!(initial_status, ConnectionStatus::Connecting),
